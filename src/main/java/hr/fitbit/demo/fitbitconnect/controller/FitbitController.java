...
public class FitbitController extends ExceptionHandlerController {
    ...
    public ResponseEntity<FitbitTokenResponse> refreshUserAccessToken(@PathVariable("user_id") @ApiParam(value = "user_id", required = true) UUID userId) {
        LOG.info("Refresh token for user={}", userId);

        final FitbitTokenResponse response = fitbitService.getRefreshToken(userId);
    ...
    public ResponseEntity<FitbitUserActivityResponse> getUserActivities(@PathVariable("user_id") @ApiParam(value = "user_id", required = true) UUID userId) {
        LOG.info("Get activities for user={}", userId);

        final FitbitUserActivityResponse response = fitbitService.getUserActivities(userId);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
}
