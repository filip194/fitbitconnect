...
public class FitbitService {
    ...
    public boolean registerUserOnFitbit(String username) {
        final ResponseEntity<FitbitTokenResponse> responseEntity = fitbitClient.requestToken(code);
        LOG.info("CODE={}", code);
        checkIfStatusCodeOK(responseEntity);

        final FitbitTokenResponse response = responseEntity.getBody();
    ...
    @Transactional
    public FitbitTokenResponse getRefreshToken(UUID userId) {
        ...
        final ResponseEntity<FitbitTokenResponse> responseEntity = fitbitClient.refreshUserToken(refreshToken,
    ...
    @Transactional
    public FitbitUserActivityResponse getUserActivities(UUID userId) {
        final UserEntity userEntity = checkIfUserExists(userId);
        checkIfUserRegisteredOnFitbit(userEntity);

        ResponseEntity<FitbitUserActivityResponse> responseEntity = null;
