...
public class FitbitServiceTest {
    ...
    public void shouldGetRefreshToken() {
        ...
        // refresh
        final FitbitTokenResponse tokenResponseRefreshed = FitbitFixture.createFitbitTokenResponseRefreshed();
        when(fitbitClient.refreshUserToken(anyString(), anyInt())).thenReturn(new ResponseEntity<>(tokenResponseRefreshed, HttpStatus.OK));

        final FitbitTokenResponse refreshTokenResponse = fitbitService.getRefreshToken(entity.getUserId());
    ...
    public void shouldGetUserActivities() {
        ...
        // get activities
        final FitbitUserActivityResponse response = FitbitFixture.createFitbitActivityResponse();
        when(fitbitClient.getUserActivities(any(), anyString())).thenReturn(new ResponseEntity<>(response, HttpStatus.OK));

        final FitbitUserActivityResponse activityResponse = fitbitService.getUserActivities(entity.getUserId());
